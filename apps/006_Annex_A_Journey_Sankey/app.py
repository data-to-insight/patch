...
st.write(
    "This tool allows you to visualise the journeys children and young people take through stages of \
...
st.header(
    "This tool performs lots of complex calculations, give it time to run. It may take minutes between reloads."
)
st.write(
    "When making changes by the widgets in the sidebar, make all the changes you want, then wait until the page reloads."
)
...
labs = pd.DataFrame(list(zip(names, labels)), columns=["name", "lab"])


journey_events = {
    "contact": {"contacts": "contact_dttm"},
...
def file_upload_fn(uploaded_files):
    if len(uploaded_files) > 1:
        ...
        for file in loaded_files.items():
            ...
            if ("list 4" in file[0].lower()) | ("assess" in file[0].lower()) & ~(
                "early" in file[0].lower()
            ):
                data_dict["assessments"] = file[1]
            if ("list 5" in file[0].lower()) | ("47" in file[0].lower()):
                data_dict["section_47"] = file[1]
            if ("list 6" in file[0].lower()) | (
                ("need" in file[0].lower()) & ("child" in file[0].lower())
            ):
                data_dict["children_in_need"] = file[1]
                print(data_dict["children_in_need"])
            if (
                ("list 7" in file[0].lower())
                | ("protection" in file[0].lower())
                | ("cpp" in file[0].lower())
            ):
                data_dict["child_protection_plans"] = file[1]
            if ("list 8" in file[0].lower()) | ("care" in file[0].lower()) & ~(
                "leaver" in file[0].lower()
            ):
        ...
        for key in keys_list:
            ...
            if ("list 4" in key.lower()) | ("assess" in key.lower()) & ~(
                "early" in key.lower()
            ):
            ...
            if (
                ("list 7" in key.lower())
                | ("protection" in key.lower())
                | ("cpp" in key.lower())
            ):
                data_dict["child_protection_plans"] = loaded_file[key]
            if ("list 8" in key.lower()) | ("care" in key.lower()) & ~(
                "leaver" in key.lower()
            ):
    ...
    data_dict["contacts"].rename(
        columns={
            data_dict["contacts"].columns[0]: "child unique id",
    ...
    data_dict["early_help_assessments"].rename(
        columns={
            data_dict["early_help_assessments"].columns[0]: "child unique id",
    ...
    data_dict["referrals"].rename(
        columns={
            data_dict["referrals"].columns[0]: "child unique id",
    ...
    data_dict["assessments"].rename(
        columns={
            data_dict["assessments"].columns[0]: "child unique id",
    ...
    # got here
    data_dict["section_47"].rename(
        columns={
            data_dict["section_47"].columns[0]: "child unique id",
    ...
    data_dict["children_in_need"].rename(
        columns={
            data_dict["children_in_need"].columns[0]: "child unique id",
    ...
    data_dict["child_protection_plans"].rename(
        columns={
            data_dict["child_protection_plans"].columns[0]: "child unique id",
    ...
    data_dict["children_in_care"].rename(
        columns={
            data_dict["children_in_care"].columns[0]: "child unique id",
    ...
    data_dict["care_leavers"].rename(
        columns={
            data_dict["care_leavers"].columns[0]: "child unique id",
...
def flag_types(df, t):
    ...
    df[("is_" + t)] = df["type"] == t
    df[("id_cum_" + t)] = (
        df[("is_" + t)].astype("int").groupby(df["id"]).transform("cumsum")
    )
    df[("id_num_" + t)] = (
        df[("is_" + t)].astype("int").groupby(df["id"]).transform("max")
    )
    return df
...
def clean_up_NFAs(dta):
    ...
    if (
        ("y" in dta.iloc[0]["referral nfa?"].lower())
    ...
    dta["was the child assessed as requiring la children’s social care support?"] = dta[
        "was the child assessed as requiring la children’s social care support?"
    ].fillna("")
...
def drop_fake_cin(dta):
    ...
    if len(cl[0]) > 0:
        ...
        num_days = (
            dta.loc[dta.index[cli], "date"] - dta.loc[dta.index[cini], "date"]
        ).days
        dta["check"] = num_days
...
uploaded_files = st.file_uploader(
    "Upload annex a here as set of csvs or single excel file:",
...
if len(uploaded_files) > 0:
    ...
    ref_dta = ref_dta.rename(
        columns={
            "number of referrals in last 12 months": "num_ref",
    ...
    with st.sidebar:
        source_option = st.sidebar.multiselect(
            ...
            options=(df["ref_source"].unique()),
            default=(df["ref_source"].unique()),
        )
    ...
    with st.sidebar:
        num_of_refs = st.sidebar.slider(
            "Number of referrals within 12 months",
            min_value=0,
            max_value=int(df["num_ref"].max()),
            value=[0, int(df["num_ref"].max())],
        )
    df = df[
        (df["num_ref"].astype(int) >= num_of_refs[0])
        & (df["num_ref"].astype(int) <= num_of_refs[1])
    ]
    ...
    df["is_cpp_lac"] = ((df["is_cpp_start"] == 1) | (df["is_lac_start"] == 1)).astype(
        "int"
    )
    df["is_cin_cpp_lac"] = (
        (df["is_cpp_start"] == 1)
