...
def owner_movements(df, col_1, col_2):
    existing_df = df[
        ((df[col_1] == "Existing") | (df[col_1] == "New")) & (df[col_2] == 1)
    ]
...
def line_chart(data_frame, var_x, var_y, var_color, var_title):
    fig = px.line(data_frame, x=var_x, y=var_y, color=var_color, title=var_title)
    st.plotly_chart(fig)


data1 = open_url(
    "https://raw.githubusercontent.com/data-to-insight/patch/main/apps/007_ofsted_market_analysis/Local%20authorities%20and%20regions.csv"
)
...
st.markdown(
    "* This tool analyses a list of social care settings providers as provided by Ofsted."
)
st.markdown("* Use the sidebar selectors to filter by geography and provider type.")
st.markdown(
    "* Click on the tabs below the table to view different breakdowns for the selected filters."
)

# Takes "Ofsted Social care providers list for LAs"
uploaded_files = st.file_uploader(
    "Upload Ofsted social care providers list:", accept_multiple_files=True
)
...
if uploaded_files:
    ...
    # Replace values
    df["Overall effectiveness"] = df["Overall effectiveness"].replace(
        "Requires Improvement", "Requires improvement to be good"
    )
    df["CYP safety"] = df["CYP safety"].replace(
        "Requires Improvement", "Requires improvement to be good"
    )
    df["Leadership and management"] = df["Leadership and management"].replace(
        "Requires Improvement", "Requires improvement to be good"
    )
    ...
    df["Total number of settings with this owner"] = df.groupby("Owner name")[
        "Setting name"
    ].transform("count")
    ...
    with st.sidebar:
        toggle = st.sidebar.radio(
            "Analyse by geographic area or owner name",
    ...
    if toggle == "Geographic area":
        ...
        with st.sidebar:
            geography_level = st.sidebar.radio(
                "Select geography level", ("England", "Region", "Local authority")
            )
        ...
        if geography_level == "Local authority":
            with st.sidebar:
                la_option = st.sidebar.selectbox(
                    "Select local authority", (local_authorities), key=1
                )
            df = df[df["Local authority"] == la_option]
            geographic_area = la_option
        elif geography_level == "Region":
            with st.sidebar:
                region_option = st.sidebar.selectbox("Select region", (regions), key=1)
        ...
        with st.sidebar:
            sector_select = st.sidebar.multiselect(
                "Select sector(s)", (sectors), default=["Private", "Local Authority"]
            )
    ...
    owner_appearances = owner_appearances.pivot(
        index="Owner name", columns="Ofsted date", values="Count"
    )
    ...
    owner_appearances.iloc[:, 2] = owner_appearances.iloc[:, 2].apply(
        lambda row: "Existing" if row == 1 else "None"
    )
    for i in range(len(months_list)):
        if i > 0:
            owner_appearances[months_list[i]] = owner_movements(
                owner_appearances, months_list[i - 1], months_list[i]
            )
    ...
    with tab1:
        ...
        count_settings = (
            df.groupby(["ofsted_date_order", "Ofsted date", "Sector"])
            .count()
            .reset_index()
        )
        ...
        total_places = (
            df.groupby(["ofsted_date_order", "Ofsted date", "Sector"])
            .sum("Number of registered places")
            .reset_index()
        )
        total_places = total_places.rename(
            columns={"Number of registered places": "Total registered places"}
        )
        # total_places = total_places.sort_values(['ofsted_date_order'])
        # st.dataframe(total_places)
        plot_chart(
            data_frame=total_places,
            var_x="Ofsted date",
        ...
        st.markdown(
            "The chart below shows the number of settings grouped by the of registered places in the setting. You can alter the size of each group by toggling the numbers in these three boxes:"
        )
        ...
        df["Registered places group"] = df["Number of registered places"].transform(
            lambda x: (
                "1 to " + str(ceiling1)
                if x < floor2
                else (
                    str(floor2) + " to " + str(ceiling2)
                    if floor2 <= x < floor3
                    else (
                        str(floor3) + " to " + str(ceiling3)
                        if floor3 <= x < floor4
                        else str(floor4) + "+"
                    )
                )
        ...
        grouped_places = (
            df.groupby(["ofsted_date_order", "Ofsted date", "Registered places group"])
            .count()
            .reset_index()
        )
        line_chart(
            data_frame=grouped_places,
            var_x="Ofsted date",
            var_y="URN",
    ...
    with tab2:
        ...
        count_settings = (
            df.groupby(["ofsted_date_order", "Ofsted date", "Overall effectiveness"])
            .count()
            .reset_index()
        )
        # count_settings = count_settings.sort_values(['ofsted_date_order'])
        ...
        count_settings_private = (
            df[df["Sector"] == "Private"]
            .groupby(["ofsted_date_order", "Ofsted date", "Overall effectiveness"])
            .count()
            .reset_index()
        )
        ...
        plot_chart(
            data_frame=count_settings_private,
            var_x="Ofsted date",
            var_y="URN",
            var_color="Overall effectiveness",
            var_title=title_4,
    ...
    with tab3:
        ...
        count_settings = (
            df.groupby(["ofsted_date_order", "Ofsted date", "CYP safety"])
            .count()
            .reset_index()
        )
        ...
        count_settings_private = (
            df[df["Sector"] == "Private"]
            .groupby(["ofsted_date_order", "Ofsted date", "CYP safety"])
            .count()
            .reset_index()
        )
        # count_settings_private = count_settings_private.sort_values(['ofsted_date_order'])
    ...
    with tab4:
        ...
        count_settings = (
            df.groupby(
                ["ofsted_date_order", "Ofsted date", "Leadership and management"]
            )
            .count()
            .reset_index()
        )
        ...
        count_settings_private = (
            df[df["Sector"] == "Private"]
            .groupby(["ofsted_date_order", "Ofsted date", "Leadership and management"])
            .count()
            .reset_index()
        )
        # count_settings_private = count_settings_private.sort_values(['ofsted_date_order'])
    ...
    with tab5:
        ...
        for need in needs_list:
            df_yes = (
                df.groupby(["ofsted_date_order", "Ofsted date", need])
                .count()
                .reset_index()
            )
            ...
            df_yes = df_yes[
                ["Ofsted date", "ofsted_date_order", "Category_of_need", "URN"]
            ]
    ...
    with tab6:
        ...
        count_owners_long = pd.melt(
            count_owners, id_vars=["Month"], value_vars=["Existing", "New", "Left"]
        )
        owner_group_dict = {"Existing": "group1", "New": "group1", "Left": "group2"}
        count_owners_long["Group"] = count_owners_long["variable"].map(owner_group_dict)
        count_owners_long = count_owners_long.rename(
            columns={"variable": "Market status", "value": "Number of owners"}
        )
