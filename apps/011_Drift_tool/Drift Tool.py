...
st.markdown(
    "This dashboard takes historical data about children's Referral, Child Protection plan and CLA status dates and uses it to visualise drift. \
...
class Drift_Data:
    ...
    def _data_clean(self, df, name):
        df.rename(
            columns={df.columns[0]: "person_id", df.columns[1]: f"{name}_date"},
            inplace=True,
        )
    ...
    def _cases_per_year(self, df):
        ...
        cases_by_year = (
            ...
            .rename_axis("year")
            .reset_index(name="cases_starting_that_year")
        )
        wait_cases_by_year = pd.merge(wait_by_year, cases_by_year, on="year")
        wait_cases_by_year.columns = [
            "year",
            "average_wait",
            "cases_starting_that_year",
        ]
    ...
    def plot_wait_time_hist(self, years, df):
        ...
        fig.update_layout(
            xaxis_title=f"Time from {title}",
            yaxis_title=f"Number of children waiting this long between {years[0]} and {years[1]}",
        )
...
with st.sidebar:
    uploaded_file = st.sidebar.file_uploader(
        "Upload historical referrals, CP plan, and CLA data", accept_multiple_files=True
    )
...
if uploaded_file:
    for file in uploaded_file:
        ...
        if ("cp" in file.name.lower()) | ("child protection" in file.name.lower()):
            cp = pd.read_csv(file)
        if (
            ("cla" in file.name.lower())
            | ("lac" in file.name.lower())
            | ("looked after" in file.name.lower())
        ):
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Referral to CP Plan", "Referral to CLA", "CP Plan to CLA"]
    )

    with st.sidebar:
        years = st.sidebar.slider(
            "Years to calculate box and bar plots",
            min_value=data.time_range[0],
            max_value=data.time_range[1],
            value=[data.time_range[0], data.time_range[1]],
        )
        years_showing = st.sidebar.slider(
            "Years to display box and bar plots",
            min_value=data.time_range[0],
            max_value=data.time_range[1],
            value=[data.time_range[0], data.time_range[0] - 5],
        )
        years_hist = st.sidebar.slider(
            "Years for wait time histogram",
            min_value=data.time_range[0],
            max_value=data.time_range[1],
            value=[data.time_range[0], data.time_range[0] - 5],
        )
    ...
    with tab1:
        ...
        st.write(
            f"The bar and box plots below, as default, show only the five most recent year's outcomes. They can be zoomed out \
        ...
        st.plotly_chart(fig)
        st.write(
            f"This plot shows the average time children wait from {title}, using data from between {years[0]} to {years[1]}, \
        ...
        st.plotly_chart(fig)
        st.write(
            f"This plot shows the distribution of wait times from {title}, using data from between {years[0]} and {years[1]}, \
        ...
        st.plotly_chart(fig)
        st.write(
            f"The histogram above shows how wait times were distributed from {title} between {years_hist[0]} and {years_hist[1]}. \
    ...
    with tab2:
        ...
        fig = data.plot_wait_by_start_year_bar(
            years, data.ref_cla_wby, title, end_point
        )
        st.plotly_chart(fig)
        st.write(
            f"This plot shows the average time children wait from {title}, using data from between {years[0]} to {years[1]}, \
        ...
        st.plotly_chart(fig)
        st.write(
            f"This plot shows the distribution of wait times from {title} status, using data from between {years[0]} and {years[1]}, \
        ...
        st.plotly_chart(fig)
        st.write(
            f"The histogram above shows how wait times were distributed from {title} status between {years_hist[0]} and {years_hist[1]}. \
                 It is a useful representation of how many children wait for different lengths of time for care they need, and what the \
                 spread of wait times is."
        )
    ...
    with tab3:
        ...
        st.plotly_chart(fig)
        st.write(
            f"This plot shows the average time children wait from {title} status, using data from between {years[0]} to {years[1]}, \
                 with the year being the year each child's {end_point} status began. On the assumption that when a child has a {start_point} that \
                leads to a {end_point} status, that child needed a {end_point} status, it represents, yearly, how long on average it takes to \
                meet children's needs. "
        )
