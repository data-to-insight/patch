...
st.title("PATCh demo")
st.write(
    "This app is a demo for the PATCh tool showing off some features users can lean to code, and some of the types of functionality available. \
...
with st.sidebar:
    ...
    chart_title = st.sidebar.text_input(
        label="What do you want to call the chart?", value="Please name the chart"
    )


with st.sidebar:
    years = st.sidebar.slider(
        "Year select", min_value=2013, max_value=2022, value=[2013, 2022]
    )

df = df[
    (df["time_period"].astype(int) >= years[0])
    & (df["time_period"].astype(int) <= years[1])
]


with st.sidebar:
    cat_option = st.sidebar.selectbox("What data category would you like?", (category))
...
with st.sidebar:
    ...
    cat_type_option = st.sidebar.selectbox(
        "What sub-category would you like?", (category_type)
    )
