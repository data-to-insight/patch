...
st.write(
    "A Python deployment of the excel disproportionality tool allowing the comparison of ethnic breakdowns between LAs (as reported by the DFE), and numbers input by the user.\
...
def denominator(LA, Date):
    ...
    LAN = data[
        (data["la_name"] == LA) & (data["time_period"] == Date)
    ]  # slices census according to setup variables
    ...
    LAdf = (
        {}
    )  #  It's always best practice to populate a dictionary then convert it to a dataframe.
    for SG in Subgroups:
        for i in SG:
            ...
            tots = LAN[
                (LAN["ethnicity"] == i) & (LAN["phase_type_grouping"] == "Total")
            ]
    ...
    LAWhite = LAN[
        (LAN["ethnicity"].isin(W_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]  #  Takes rows of LAN where the ethnicity column is in the W_ind list, and phase type grouping is Total.
    Wtot = LAWhite["headcount"].sum()  #  Sums the headcount column of the above rows.
    LAdf["White Total"] = (
        Wtot  #  Returns the above value to the LAdf dictionary with the key White Total.
    )

    LAAsian = LAN[
        (LAN["ethnicity"].isin(A_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]
    Atot = LAAsian["headcount"].sum()
    LAdf["Asian Total"] = Atot

    LABlack = LAN[
        (LAN["ethnicity"].isin(B_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]
    Btot = LABlack["headcount"].sum()
    LAdf["Black Total"] = Btot

    LAMixed = LAN[
        (LAN["ethnicity"].isin(M_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]
    Mtot = LAMixed["headcount"].sum()
    LAdf["Mixed Total"] = Mtot

    LAOther = LAN[
        (LAN["ethnicity"].isin(O_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]
    Otot = LAOther["headcount"].sum()
    LAdf["Other Total"] = Otot

    LAUnclassified = LAN[
        (LAN["ethnicity"].isin(U_ind)) & (LAN["phase_type_grouping"] == "Total")
    ]
...
def calcs_for_plots():
    ...
    for i in range(len(comparison_df)):
        if (
            comparison_df["denominator"].iloc[i] != 0
        ):  # checks to see no division by 0 will happen
            v = round(
                (
                    (
                        comparison_df["numerator"].iloc[i]
                        / comparison_df["denominator"].iloc[i]
                    )
                    * 100
                ),
                1,
    ...
    for i in range(len(comparison_df)):
        v = round(
            comparison_df["Rate per 10,000"].iloc[i]
            / comparison_df["Rate per 10,000"].iloc[0],
            2,
        )
        RRI.append(v)
    ...
    for i in range(len(comparison_df)):
        v = round(
            (comparison_df["denominator"].iloc[i] / total_known_ethnicity_denominator)
            * total_known_ethnicity_numerator
        )
    ...
    for i in range(len(comparison_df)):
        v = round(
            total_known_ethnicity_numerator - comparison_df["Yes expected"].iloc[i]
        )
    ...
    ChiSquared = []
    for i in range(len(comparison_df)):
        if (comparison_df["numerator"].iloc[i] == 0) or (
            comparison_df["numerator"].iloc[i] == 0
        ):
            ChiSquared.append(0)
        else:
            observed = {
                "Yes": comparison_df["Yes observed"].iloc[i],
                "No": comparison_df["no_observed"].iloc[i],
            }
            expected = {
                "Yes": comparison_df["Yes expected"].iloc[i],
                "No": comparison_df["No expected"].iloc[i],
            }
    ...
    sig_higher_RRI = []
    for i in range(len(comparison_df)):
        if (comparison_df["RRI"].iloc[i] > 1) & (
            comparison_df["Stat Sig"].iloc[i] == "Sig"
        ):
    ...
    comparison_df["no_sig_diff_RRI_calc"] = (
        comparison_df["Sig Higher RRI"] + comparison_df["Sig Lower RRI"]
    ).to_list()
    ...
    sig_higher_RP10k = []
    for i in range(len(comparison_df)):
        if (comparison_df["Higher Lower"].iloc[i] == "Higher") & (
            comparison_df["Stat Sig"].iloc[i] == "Sig"
        ):
    ...
    sig_lower_RP10k = []
    for i in range(len(comparison_df)):
        if (comparison_df["Higher Lower"].iloc[i] == "Lower") & (
            comparison_df["Stat Sig"].iloc[i] == "Sig"
        ):
            v = comparison_df["Rate per 10,000"].iloc[i]
        else:
    ...
    comparison_df["no_sig_diff_RP10k_calc"] = (
        comparison_df["Sig Higher RP10k"] + comparison_df["Sig Lower RP10k"]
    ).to_list()
    ...
    comparison_df["no_sig_diff_PCT_calc"] = (
        comparison_df["Sig Higher PCT"] + comparison_df["Sig Lower PCT"]
    ).to_list()
...
with st.sidebar:
    time_period = st.sidebar.selectbox(
        "Select school year for school census population data",
        options=data["time_period"].unique(),
    )
    la_name = st.sidebar.selectbox(
        "Select LA for school census population data", options=data["la_name"].unique()
    )

with st.sidebar:
    comp_name = st.sidebar.text_input(
        'Name of local subgroup for comparison (e.g. "CLA")', value="sample data"
    )
...
with tab1:
    ...
    RRI_tab = comparison_df[
        ["ethnicity", "numerator", "denominator", "Rate per 10,000", "RRI"]
    ].iloc[:21]
    RRI_tab.set_index("ethnicity")
    RRI_tab.rename(
        columns={
            "numerator": "Input numbers",
            "denominator": str(time_period) + " Population",
        },
        inplace=True,
    )
    ...
    for i in range(
        len(comparison_df.iloc[:21])
    ):  #  Slices comparison_df to only select sub-groups.
        if (
            comparison_df["Chart Titles"].iloc[i] == "WBRI"
        ):  #  Returns black if white british.
            hue = "black"
        elif (
            comparison_df["No Sig Diff (RRI)"].iloc[i] > 0
        ):  #  elif means other colours will only be returned if the WBRI check was not True.
            hue = "grey"
    ...
    ax = sns.barplot(
        data=comparison_df.iloc[:21], x="Chart Titles", y="RRI"
    )  #  Barplot with x and y values set.
    ...
    ax.set_xlabel(
        "Red = Sig higher, Blue = Sig lower, Grey = No sig diff"
    )  #  Uses x titles to set key to keep the plot tight.
    ax.set_title(
        f"RRI of minority ethnic sub-groups in {comp_name} compared to White British in {la_name} in census period {time_period}"
    )
...
with tab2:
    ...
    ax.set_title(
        f"RRI of minority ethnic main groups in {comp_name} compared to White British in {la_name} in census period {time_period}"
    )
...
with tab3:
    RP10k_tab = comparison_df[
        ["ethnicity", "numerator", "denominator", "Rate per 10,000", "Higher Lower"]
    ].iloc[:21]
    ...
    fig, ax = plt.subplots()
    ax = sns.barplot(
        data=comparison_df.iloc[:21], x="Chart Titles", y="Rate per 10,000"
    )
    plt.xticks(rotation=90)
    ax.set_ylabel("Rater per 10,000")
    ax.set_xlabel("Red = Sig higher, Blue = Sig lower, Grey = No sig diff")
    ax.set_title(
        f"Rate per 10,000 population of ethnic sub-groups in {comp_name} compared to {la_name} in census period {time_period}"
    )
...
with tab4:
    ...
    fig, ax = plt.subplots()
    ax = sns.barplot(
        data=comparison_df.iloc[21:27], x="Chart Titles", y="Rate per 10,000"
    )
    plt.xticks(rotation=90)
    ax.set_ylabel("Rate per 10,000")
    ax.set_xlabel("Red = Sig higher, Blue = Sig lower, Grey = No sig diff")
    ax.set_title(
        f"Rate per 10,000 of main ethnic groups in {comp_name} compared to {la_name} in census period {time_period}"
    )
...
with tab5:
    PCTTab = comparison_df[
        ["ethnicity", "numerator", "denominator", "Percentages", "Higher Lower"]
    ].iloc[:21]
    ...
    ax.set_xlabel("Red = Sig higher, Blue = Sig lower, Grey = No sig diff")
    ax.set_title(
        f"Percentage of ethnic subgroups in {comp_name} compared to {la_name} in census period {time_period}"
    )
...
with tab6:
    ...
    ax.set_xlabel("Red = Sig higher, Blue = Sig lower, Grey = No sig diff")
    ax.set_title(
        f"Percentage of main ethnic groups in {comp_name} compared to {la_name} in census period {time_period}"
    )
